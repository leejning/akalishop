package com.akali.business.member.cart.web;

import com.akali.business.member.cart.api.CartControllertApi;
...
public class CartControllert implements CartControllertApi {
    ...
    @GetMapping("{page}")
    @Override
    public QueryResponseResult<CartItemDTO> getCart(@PathVariable Integer page, HttpServletRequest request){
        Long memberId = Oauth2JwtUtil.getLoginMemberId(request);
        ...
        return QueryResponseResult.SUCCESS(response.getData()).message("获取购物车");
    }
}
