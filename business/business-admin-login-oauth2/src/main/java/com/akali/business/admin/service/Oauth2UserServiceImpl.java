...
import com.akali.business.admin.bean.AdminUser;
...
public class Oauth2UserServiceImpl implements UserDetailsService {
    ...
    @Override
    public UserDetails loadUserByUsername(String adminAccount) throws UsernameNotFoundException {
        DubboResponse<AdminLoginDTO> response = adminUserService.queryAdminUserWithPermissionByAccount(adminAccount);
        if (response.isSuccess()) {
            ...
            AdminUser adminUser = new AdminUser(adminAccount, user.getPassword(), authorityList);
            adminUser.setAdminName(user.getAdminName());
