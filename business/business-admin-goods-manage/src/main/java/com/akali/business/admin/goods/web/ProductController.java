...
public class ProductController implements ProductControllerApi {
    ...
    @GetMapping("{page}")
    @Override
    public QueryResponseResult<SpuPageDTO> queryProductPage(@RequestBody SpuQueryDTO spuQueryDTO) {
        DubboResponse<QueryResult<SpuPageDTO>> response = productService.queryProductPage(spuQueryDTO);
        if (!response.isSuccess()) {
            ExceptionCast.cast(response.getResultCode());
        }
        return QueryResponseResult.SUCCESS(response.getData());
